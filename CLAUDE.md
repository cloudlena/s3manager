...
**Auth** (`internal/app/s3manager/auth/`): Optional login in front of the whole app, independent of the S3 credentials. `Authenticator` is the extension point — `NewAnonymous` backs `AUTH_PROVIDER=none`, `NewOIDC` implements the authorization code flow with PKCE via `coreos/go-oidc` and `x/oauth2`. The result lives in an encrypted `gorilla/sessions` cookie, so the app stays stateless. `auth.Middleware` puts an `Identity` (with a `Role`) into the request context; `auth.RequireRole` guards the mutating routes.

Roles only ever subtract: `Options` is a global cap, so `effectiveOptions` (`permissions.go`) ANDs each capability with the role and never assigns from it, and a route the configuration disables is not registered at all. `permissions_internal_test.go` pins that invariant.
...
HTTP request
  → Gorilla mux (main.go)
  → auth.Middleware (identity + role into the context; unauthenticated → /auth/login)
  → auth.RequireRole (mutating routes only)
